...
{   
    ...
    public sealed class PayflowPaymentService
    {
        #region Constants
                ...
                string ip, string userAgent, string surferID, ref CreditCardPaymentResult result)
        {
            try
            {
                ...
                if (transactionType == Constants.Enumerators.TransactionType.Sale)
                {
                    ...
                    result = PayflowHelper.DoSale(ccNum, csc, updatedCc, amount, description, comment1, comment2, ip, userAgent);
                }
                else
                {
                    // Perform authorization transaction using the actual ccNum
                    result = PayflowHelper.DoAuthorization(ccNum, csc, updatedCc, amount, description, comment1, comment2, ip, userAgent);
                }
